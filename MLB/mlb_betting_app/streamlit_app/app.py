...
.explainer-card {
    ...
    background: linear-gradient(180deg, rgba(14,165,233,.12), rgba(37,99,235,.075));
    border-radius: 18px;
    padding: 1rem 1.1rem;
...
def render_card_explainer() -> None:
    st.markdown(
        """
        <div class="explainer-card">
          <div class="explainer-title">How to read the cards</div>
          <div class="explainer-body">
            The model separates <b>who is most likely to win</b> from <b>which side is priced well enough to bet</b>.
            A team can be the value bet even when it is not the model favorite.
          </div>
          <div class="explainer-grid">
            <div class="explainer-item"><b>Model Favorite</b><br>The team the model thinks is most likely to win the game.</div>
            <div class="explainer-item"><b>Value Bet</b><br>The recommended betting side after comparing model probability to sportsbook price.</div>
            <div class="explainer-item"><b>Recommended Team Win Prob</b><br>The model's win probability for the recommended value-bet side.</div>
            <div class="explainer-item"><b>Market Break-Even</b><br>The win probability required to break even at the listed moneyline price.</div>
            <div class="explainer-item"><b>Edge</b><br>Model probability minus market break-even probability for the value-bet side.</div>
            <div class="explainer-item"><b>EV</b><br>Expected value per unit wagered. Positive EV means the price is favorable if the model is accurate.</div>
          </div>
          <div class="explainer-example">
            <b>Example:</b> If the Cubs are the model favorite at 54%, the Giants still win 46% of the time.
            If the Giants are +150, their market break-even is about 40%, so a 46% model probability creates value even though the Cubs are more likely to win.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_bet_card(row: pd.Series, rank: int) -> None:
    ...
    chips = [
        ...
        f'<span class="chip">Model Favorite {model_favorite} · {_fmt_pct(favorite_prob)}</span>',
    ]
    if model_prob is not None and not pd.isna(model_prob):
        chips.append(f'<span class="chip">Recommended Team Win Prob {_fmt_pct(model_prob)}</span>')
...
def render_data_dictionary() -> None:
    dictionary = pd.DataFrame(
        [
            ...
            ("Recommended Team Win Prob", "Model-estimated probability that the recommended value-bet side wins. Only shown when there is a recommendation."),
            ("Market Break-Even", "Market-implied break-even win probability for the recommended side based on the sportsbook price."),
